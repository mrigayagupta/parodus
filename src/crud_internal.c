...
int createObject( wrp_msg_t *reqMsg, wrp_msg_t **response )
{
	...
	if(reqMsg->u.crud.dest !=NULL)
	{
	   ...
	   /* tagObj = cJSON_CreateObject());
	    cJSON_AddStringToObject(tagObj, tagName, obj = cJSON_CreateObject());
            
            paramList = cJSON_CreateArray();
            
            cJSON_AddItemToArray(paramList, obj);
            cJSON_AddStringToObject(obj, objName, reqMsg->u.crud.payload);*/
            
           cJSON* response = cJSON_CreateObject();
	   cJSON_AddItemToObject(response, tagName, parameters = cJSON_CreateArray());
