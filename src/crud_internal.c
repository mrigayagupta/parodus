...
int retrieveObject( wrp_msg_t *reqMsg, wrp_msg_t **response )
{
	...
	cJSON *json = NULL;
	char *jsonData = NULL;
	char *child,*obj[5],*resValue = NULL;
	...
	if(status == 1)
		...
		{
			ParodusInfo( "cjson parse success\n" );
			   ...
			   ParodusInfo("destVal is %s\n", destVal);
				...
				if( paramArray != NULL ) 
				{
					int itemSize = cJSON_GetArraySize( paramArray );
                    			...
                    			if( itemSize == 0 ) //check this scenario to get from paroduscfg
                    			{
